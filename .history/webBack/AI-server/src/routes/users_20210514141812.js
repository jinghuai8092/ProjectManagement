...
const {register,login,reset,resetPassword}=require('../controller/user')
...
router.post('/sendEmail',async function(ctx,next){
  ...
  if(result!=null){
    ...
    ctx.body=new ErrorModel(10010,'验证用户名邮箱失败')
  }
})

router.post('/resetPassword',async function(ctx,next){
  const {username,securityCode,password}=ctx.request.body
  const result=await resetPassword(username,securityCode,password)
  if(result){
    //成功
    ctx.session.userInfo={username} //设置session
    ...
    //失败
    ctx.body=new ErrorModel(10011,'密码重置失败')
  }
})
