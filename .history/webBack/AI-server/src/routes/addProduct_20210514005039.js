...
const { createProduct, getProductList,getProductById,updateProduct,deleteProduct,getProductByInput } = require('../controller/product')
...
//  商品查询
// router.post('/search',loginCheck,async function(ctx, next){
    router.post('/search/product',async function(ctx, next){
    const input = ctx.request.bodys


    try {
        const product =await getProductByInput(input)
        ctx.body=new SuccessModel(product)
    } catch (ex) {
        console.error(ex)
